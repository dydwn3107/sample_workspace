...
public class AccountManage extends DAO{
	// 싱글톤
	private static AccountManage am = new AccountManage();//@@@@@static 맞는ㄷ지
	...
	public int updateMoney(Account account, int cmd) {
		...
		try {
			...
			if(cmd == 1) {
				...
				//balance + account.getBalance() => 입금한 후 금액
	...
	public void transferMoney(String toAccount, String fromAccount, int balance) {
		...
		try{
			...
			if(result == 1) {
				...
				//받는 사람의 계좌에 돈을 넣어주는 sql 정상출금 됬을 때 실행되기 때문에
